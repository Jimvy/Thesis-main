...
def labelLines(
    ...
    **kwargs,
    ...
    labels : list of labels
       If non-None, the list of labels to use, instead of the default ones.
    ...
    if type(xvals) == tuple:
        ...
        if xscale == "log":
            xvals = np.logspace(np.log10(xmin), np.log10(xmax), len(lab_lines) + 2)[1:-1]
        else:
    ...
    txts = []
    try:
        yoffsets = [float(yoffsets)] * len(lab_lines)
    except TypeError:
        pass
